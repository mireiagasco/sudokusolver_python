...
class CSP:
    ...
    def backtrack(self, assignment):
        ...
        for value in assignment[unassigned_var]:
            ...
            # Attempt to perform inference (AC-3) on the new assignment
        ...
        return None  # Backtrack: no valid solution found for this branch
    ...
    def select_unassigned_variable(self, assignment):
        ...
        """
        unassigned_var = None
        for key, lst in assignment.items():
            if len(lst) > 1:
                unassigned_var = key
                break
            ...
            for value in i_values:
                for value2 in assignment.get(j):
                    ...
                    if (value, value2) in self.constraints[i][j]:
                        ...
                        break
                if not satisfied:
                    assignment[i] = [x for x in i_values if x != value]
                    revised = True
